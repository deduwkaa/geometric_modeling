...
from PySide6.QtWidgets import (
    ...
    QGroupBox, QTabWidget, QPushButton, QSizePolicy, QCheckBox, QSlider
)
...
class CardioidMath:
    ...
    def get_point(a, t):
        """
        Параметричні рівняння кардіоїди:
        x = a * (1 - cos(t)) * cos(t)
        y = a * (1 - cos(t)) * sin(t)
        t - кут в радіанах [0, 2pi]
        """
        r = a * (1 - math.cos(t))
        x = r * math.cos(t)
        y = r * math.sin(t)
    ...
    def get_derivatives(a, t):
        ...
        x = a(cos(t) - cos^2(t))  -> dx/dt = a(-sin(t) + 2sin(t)cos(t)) = a(sin(2t) - sin(t))
        y = a(sin(t) - sin(t)cos(t)) -> dy/dt = a(cos(t) - cos(2t))
        """
    ...
    def calculate_properties(a, t):
        ...
        S = 6 * pi * a^2
        L = 16 * a
        R_curv = (8/3) * a * sin(t/2)  (радіус кривини)
        """
        area = 6 * math.pi * (a ** 2)
        length = 16 * a
        # Радіус кривини (у точці t)
        # Формула: ro = (4/3) * sqrt(2*a*r). Або спрощено 8a/3 * sin(t/2)
...
class CanvasWidget(QWidget):
    def __init__(self, parent=None):
        ...
        # Параметри кривої
        self.param_a = 50.0  # Розмір кардіоїди
        self.param_t = 1.0  # Положення точки для дотичної (в радіанах)
    ...
    def paintEvent(self, event):
        ...
        # 1. Налаштування системи координат (центр екрану)
        cx = self.width() / 2
        cy = self.height() / 2
        painter.translate(cx + self.offset_x, cy + self.offset_y)
        painter.scale(self.scale_factor, -self.scale_factor)  # Y вгору

        # 2. Сітка та Осі (вони не рухаються разом з кривою при трансформаціях)
        self.draw_grid(painter)

        # 3. Застосування трансформацій (зсув/обертання кривої)
        painter.setTransform(self.transform_matrix, True)

        # 4. Малювання Кардіоїди
        self.draw_cardioid(painter)

        # 5. Дотична, Нормаль і Точка
    ...
    def draw_grid(self, painter):
        ...
        pen.setWidthF(1)
        pen.setCosmetic(True)  # товщина не змінюється при зумі
        painter.setPen(pen)
        ...
        pen.setWidth(2)
        painter.setPen(pen)
        painter.drawLine(-limit, 0, limit, 0)  # X
        painter.drawLine(0, -limit, 0, limit)  # Y
    ...
    def draw_tangent_normal(self, painter):
        ...
        # Обчислюємо похідні dx, dy
        dx, dy = CardioidMath.get_derivatives(self.param_a, t_val)

        # Довжина ліній для відображення
        line_len = 100

        # --- ДОТИЧНА (Tangent) ---
        # Вектор дотичної: (dx, dy)
        # Нормалізуємо для красивої довжини
        mag = math.sqrt(dx ** 2 + dy ** 2)
        if mag > 0.001:
            tx = (dx / mag) * line_len
            ty = (dy / mag) * line_len

            pen_tan = QPen(QColor("magenta"), 2)  # Фіолетова
            pen_tan.setCosmetic(True)
            painter.setPen(pen_tan)
            painter.drawLine(QPointF(pt.x() - tx, pt.y() - ty),
                             QPointF(pt.x() + tx, pt.y() + ty))

            # --- НОРМАЛЬ (Normal) ---
            # Вектор нормалі перпендикулярний дотичній: (-dy, dx)
            nx = (-dy / mag) * line_len
            ny = (dx / mag) * line_len

            pen_norm = QPen(QColor("green"), 2)  # Зелена
            pen_norm.setCosmetic(True)
            painter.setPen(pen_norm)
            painter.drawLine(QPointF(pt.x() - nx, pt.y() - ny),
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.slider_t.setRange(0, 628)  # 0 to 2*pi (approx 6.28 * 100)
        self.slider_t.setValue(100)

        grid_curve.addWidget(QLabel("Параметр a (розмір):"), 0, 0)
        grid_curve.addWidget(self.spin_a, 0, 1)
        grid_curve.addWidget(QLabel("Точка t (кут):"), 1, 0)
        ...
        # 3. Трансформації (Зсув та Обертання)
        ...
        self.lbl_asymp = QLabel("Асимптота: Відсутня (замкнена)")
        self.lbl_asymp.setStyleSheet("color: gray; font-style: italic;")

        res_layout.addWidget(self.lbl_area)
        res_layout.addWidget(self.lbl_len)
        res_layout.addWidget(self.lbl_rad)
        res_layout.addWidget(self.lbl_asymp)
        ...
        self.anim_direction = 1  # 1 - збільшується, -1 - зменшується
    ...
    def update_all(self):
        a = self.spin_a.value()
        t_deg = self.slider_t.value() / 100.0  # Слайдер 0..628 -> радіани 0..6.28
    ...
    def toggle_animation(self, checked):
        if checked:
            self.timer.start(30)  # 30 мс інтервал
        else:
            self.timer.stop()

    def animate_step(self):
        # Анімуємо параметр 'a' (пульсація)
        ...
        new_a = current_a + (0.5 * self.anim_direction)
        self.spin_a.setValue(new_a)  # Це автоматично викличе update_all
